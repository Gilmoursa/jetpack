...
class Post_By_Email_Abilities_Test extends WP_UnitTestCase {
	...
	/**
	 * Authenticated (subscriber+) user id, created once per test.
	 *
	 * Post by Email is a per-user feature gated on is_user_logged_in(), so any
	 * authenticated role is enough to view/manage the caller's own address.
	 *
	 * @var int
	 */
	private $user_id;
	 ...
	 */
	public function test_can_view_post_by_email_allows_logged_in_user() {
	...
	public function test_can_manage_post_by_email_allows_logged_in_user() {
		wp_set_current_user( $this->user_id );
	...
	/**
	 * Remote `getPostByEmailAddress` fails → surface as
	 * `jetpack_post_by_email_service_unreachable` so callers know to retry
	 * rather than misread a null field as "the user has no address".
	 */
	public function test_get_status_errors_when_remote_fetch_fails() {
		wp_set_current_user( $this->user_id );
		$this->activate_post_by_email_module();

		Post_By_Email_Abilities_Test_Stub::reset(
			new \WP_Error( 'jetpack_post_by_email_data_unavailable', 'remote down' )
		);
